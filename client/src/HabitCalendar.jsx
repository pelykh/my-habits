...
export default function HabitCalendar() {
  ...
  useEffect(() => {
    async function load() {
      const [h, l] = await Promise.all([
        fetchHabits().catch(() => null),
        fetchSetting("lang").catch(() => null),
      ]);
  ...
  // ── Re-fetch marks when habit or month changes ──
  const habitId = habits[habitIdx]?.id;
  useEffect(() => {
    if (!loaded || habitId == null) return;
    fetchMarks(habitId, year, month)
      .then(setMarks)
      .catch(() => setMarks({}));
  }, [habitId, year, month, loaded]);

  // ── Persist lang preference ──
  useEffect(() => {
    if (!loaded) return;
    saveSetting("lang", lang).catch(() => {});
  }, [lang, loaded]);
  ...
  const handleCommit = (d, strokes, tl) => {
    ...
    setMarks(prev => ({ ...prev, [dk]: { strokes, tool: tl } }));
    commitMark(habit.id, dk, strokes, tl).catch(console.error);
  };
  ...
  const handleAdd = async ({ cue, routine, reward }) => {
    try {
      ...
      const newIdx   = habits.length;
      const created  = await createHabit({ cue, routine, reward, colorIdx });
  ...
  const handleImport = (e) => {
    ...
    reader.onload = async (ev) => {
      try {
        ...
        setHabitIdx(0);
        // marks will re-fetch via the habitId/month useEffect
    ...
    try {
      if (habits.length === 1) {
        ...
        const created = await createHabit({
          cue: DEFAULT_HABITS[0].cue,
          routine: DEFAULT_HABITS[0].routine,
          reward: DEFAULT_HABITS[0].reward,
          colorIdx: DEFAULT_HABITS[0].colorIdx,
        });
        setHabits([created]);
        setHabitIdx(0);
      } else {
      ...
      <div style={{ width: "100%", maxWidth: 530 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 5 }}>
          {grid.map((d, i) => {
            ...
            const mark     = marks[dayKey(year, month, d)];
